...
| **飞书个人聊天**(发给自己的消息) | 当前唯一的副本 | 你 + 飞书平台;企业账号还可能包括管理员/合规审计 |
...
2. 在飞书里**收藏/固定**那条消息(不是靠滚动找)。发的时候带上一句可搜的描述:
   `EasyAndroid 签名凭据 (release, SHA eb3bfaf6)` —— 半年后你是**搜**它,不是翻聊天记录。
...
> ⚠️ **别把「单行长行」的凭据包粘进终端 —— 会被静默截断。**
>
> Linux 终端在规范模式下单行缓冲上限是 **4096**(`MAX_CANON`)。实测把 7001 个字符
> 的一行粘进去,**只有 4096 个进来了**,其余静默丢掉。
>
> 旧的导出格式就是「10 行注释 + 一个 6800 字符的长行」,正好踩中这个坑 ——
> 粘进终端必然截断,而且看起来像"备份坏了",其实是**传输方式**坏了。
>
> 现在 `--export` 默认**折行 76 列**(base64 惯例),经终端粘贴实测
> 7393 字节进 / 7393 字节出,完整无损。解析侧也认得出这种情况:
>
> ```
> !! 有超长行(>4000 字符),几乎肯定是**粘进终端时被截断了**。
>    Linux 终端规范模式下单行缓冲上限是 4096(MAX_CANON),长行会被静默切掉。
> ```
>
> **手上那份旧格式的怎么验**:别粘进终端,用记事本粘进去另存为 UTF-8,
> 再 `--drill <那个文件>` —— 文件没有这个限制。或者重新导出一份折行格式,
> 替换飞书里那条消息。

解析器**故意做得宽容**:折行(哪怕折到 20 列)、发送者/时间戳、末尾粘在负载上的
「已读」、BOM、CRLF 都能处理;它按行取 base64 前缀 + 用「完整解压 + tar 里必须有
`release.jks` 和 `keystore.properties`」来验证,而不是只看格式。

真被截断或改动了,它会明确告诉你,而不是让你拿一份坏备份等到需要时才发现:

```
!! feishu.txt 里的内容拼起来解不出 gzip —— **不完整或被改动了**。
   最可能是复制时截断(聊天客户端常只展开前面一段)。
   重新整段复制,再用 --drill 验一次。
```

> ⚠️ 另外:GitHub Secret **不算备份**。它的值只能写、永远读不回来,所以它只是
> 「CI 能用」而不是「你还能拿到」。
