...
| **飞书个人聊天**,以**文件消息**形式发的 `signing-bundle.b64` | 当前唯一的副本 | 你 + 飞书平台;企业账号还可能包括管理员/合规审计 |
| GitHub Secret `KEYSTORE_B64`(EasyIndie/EasyAndroid) | 给 CI 用 | 仓库管理员 |
| — 密码管理器 — | ⚠️ **还没有** | — |
| — 离线冷备 — | ⚠️ **还没有** | — |

> 「上次验证」的确切记录在 [`signing-manifest.txt`](../signing-manifest.txt)
> 的 `drill.*` 三行里(由 `--record` 自动写),以那里为准,别在这张表里重复维护。

#### 飞书个人聊天当凭据库:能用,但要知道你在换什么

聊天消息有它的好处 —— 同步到所有设备、能搜、不容易丢。代价是三条具体的:

| 风险 | 说明 |
|---|---|
| **平台可读** | 消息存在飞书服务器上。企业飞书里,管理员的合规/审计工具**能检索成员消息**,DLP 也可能扫描内容。个人版宽松些,但本质一样:你不是唯一能读到它的人。 |
| **账号即单点** | 换工作、账号被停用、离职交接 = 凭据一起没了。密码管理器至少是你自己的资产。 |
| **复制会失真** | 聊天客户端会折行、会在末尾追加东西(「已读」、时间戳)、有时只展开一段。`--drill` 能测出失真,但前提是你真去测。 |

**三条具体建议:**

1. **把它挪一份进密码管理器**,密码管理器那份当权威,飞书这份当"顺手能找到的"。这不是洁癖 ——
   飞书那份的存亡取决于你的账号,而密码管理器那份取决于你自己。
2. **文件名就是你在飞书里的搜索依据。** 别用 `signing-bundle.b64` 这种通用名,
   发之前改成能搜的:
   `EasyAndroid-签名凭据-release-eb3bfaf6-2026-09-17.b64`
   然后**收藏/固定**那条消息(不是靠滚动找)。半年后你是**搜**它,不是翻聊天记录。
3. **别发到群、别放进共享文档、别发给任何人**(包括同事)。个人聊天里发给自己的消息是这里面最不容易被误读的形态。

#### 怎么验飞书那份还是好的

飞书里那份是**文件消息**,不是文本 —— 这是个好选择:

| | 文件消息 | 粘贴成文本 |
|---|---|---|
| 字节完整性 | **原样**,不会折行/追加 | 客户端会折行、会追加「已读」、可能只展开一段 |
| 终端 4096 限制 | 不涉及 | 单行长行会被静默截断 |
| 版本混淆 | 文件名可带版本 | 容易和注释混在一起 |

所以验证很简单 —— **把文件下载到本机,然后演练**:

```bash
  # 1. 在飞书里把那个文件下载到 E:/EasyAndroid/(或任意目录)
# 2. 演练。--label 把「验的是飞书那份」记进 manifest,与本地文件区分开
bash tools/gen-keystore.sh --drill signing-bundle.b64 --record \
     --label "飞书个人聊天(文件消息)"

# 忘了存哪、或者文件名被客户端改过,就指定完整路径:
bash tools/gen-keystore.sh --drill /mnt/e/EasyAndroid/signing-bundle.b64 --record \
     --label "飞书个人聊天(文件消息)"
```

通过就说明那份**现在**能还原出发布用的那一把。半年后重跑一次 ——
备份会悄悄过期(文件被云盘改坏、当时设的口令忘了、存的是更早那次的版本)。

> 解析器对粘贴文本也是宽容的(折行、噪音、BOM/CRLF),所以万一哪天你只能
> 复制文本而不是拿文件,也能验。但**优先用文件**:少一层不确定性。
>
> ⚠️ 一个和文件消息有关的坑:Linux 终端规范模式单行上限 4096(`MAX_CANON`)。
> 如果你把凭据包**粘贴进终端**(而不是下载文件),单行长行会被静默截断,
> 看起来像"备份坏了"。现在 `--export` 默认折行 76 列,并已实测经终端粘贴
> 7393 字节进 / 7393 字节出完整无损;真被截断时探针也会明确指出来。
